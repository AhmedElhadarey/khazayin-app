...
export type NavIconProps = IconProps & {
  ...
  active?: boolean;
  /** Accent applied to the active variant's cut-out (the home icon's door). */
  accentColor?: string;
};
...
export function NavHomeIcon({
  ...
  return (
    <Svg width={size} height={size} viewBox={VIEW_BOX} fill="none">
      ...
      />
      {active ? (
        <Rect x={11} y={13.4} width={2} height={4.6} rx={1} fill={accentColor ?? color} />
      ) : (
        <Path
          d="M12 15.5v2.5"
          stroke={color}
...
export function NavLibraryIcon({
  ...
  return (
    <Svg width={size} height={size} viewBox={VIEW_BOX} fill="none">
      ...
      {active ? null : (
        <>
          <Path d="M7.1 9.2h3" stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" />
          <Path d="M7.1 12h3" stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" />
        </>
      )}
    </Svg>
  );
 ...
 */
export function NavSectionsIcon({
  size = 24,
  color,
  strokeWidth = 1.6,
  active,
