...
//
// Content is the deterministic mock set in data/content/exclusive.ts: there
...
export default function ExclusiveScreen() {
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['top', 'bottom']}>
      <InlineHeader title="حصريات خزائن الرحمن" onBack={() => router.back()} />
      <View style={styles.searchBlock}>
        <SearchPill placeholder="بحث.." onPress={() => router.push('/search' as any)} />
      </View>
      <View style={styles.body}>
        <FlatList
          data={EXCLUSIVE_ITEMS}
          ...
          renderItem={({ item }) => (
            <LectureCard
              ...
              compact
              iconNode={<SparkleBadge size={CARD_DENSITY.lectureBadgeDisc} />}
              title={item.title}
